...
public class Test2001 {
        ...
        public void Check() throws Exception {
            for (String s : results) {
                if (!s.equals("from SUBCLASS: Hello from " + getName())
                        ...
                        && !s.equals("from SUBCLASS: Hello World, Bonjour le Monde, " +
                                             "Hej Verden, こんにちは世界 from " + getName())) {
    ...
    public static void doTest() throws Exception {
        if (!VMRuntime.getRuntime().is64Bit()) {
            // Reduce the task count limit on 32-bit VMs. Since we create a class loader
            // for each task, we create too many linear alloc spaces. On 32-bit systems
            // this might cause an OOM because we run out of virtual address space.
