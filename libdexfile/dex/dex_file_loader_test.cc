...
// Taken from art-gtest-jars-MultiDexContainer.jar
static const char kRawContainerDex[] =
  "UEsDBBQACAAIAAAAITgAAAAAAAAAAAAAAAALAAAAY2xhc3Nlcy5kZXhsk8GLI0UUxr/qqmSTzW4m"
  "O3EPimKmBBFJZhI3m50kIgoyp+gIAxEUweruSqYz6e7Q3Rmi4jIiisc9LLIIwooHD+pxwYN/gIIH"
  "L94XvK0nQdi9CFJd1ZOeceuQenmv6716X/2eK1cX290OsDX6df/RX59985B9+LT7YMY+vffak8+y"
  "v7sWsAKwGnc3YdZtAlwF8IMFlKDjDMAdAATALwCKAH43//WPXj8X9G4BoOZcwXx/AUAFwGVzhJkz"
  "FRNX65KxV2RtnxCVw8IV5ItR1NJ/5NS7kda1sJlGc5cyd1GrZPbPTZiYDBOi77OoqdtWwUDSHmZE"
  "5+UgWNSUxyZAUFP5LllBg2ELVVxBFXWyUX2H4gnrS+T6I6ZvCzg5YXctUv7Jugyc6cTCWmea05rl"
  "9C7kNC/mdCcNfb5BAGps1QupaVvpiIb+Pu25pm31xmrP3tlqaPsOBTJmkq+/S958sPVn9Mnz82Tv"
  "j+r4YfvfPf7VrS/o/5npFta5mBFc1a9lwpveXqC67qs5v2JGcahq36XAtxT4ngL3KPAbBe5T4B8K"
  "lBhQZ8AzDHiRAS0GdBiwx4ARA95mwHsMsBlwxICY6ZqZdhWjm2XqVnJc5CHeyHF1PnrR7M+dcq4j"
  "t6jO9RTKKacZV7dpxpWVclU3tBRf9gIveQVkhOLoDeEFQ5RGB9IJA3eIq6OZOBY7XrjzVuQFyUES"
  "SeEPsandcxFMd/btmXSSs76DJPKC6TnfB3Ei/SHKqsa28qO0H3lTLxBzVHRB7SZjWOMR6u8+JiMT"
  "0TRGeSoTfQLMF14AGi4TXFioO84DFGMd+5HcvPn67kfcFs6RDFw+4K5c8SZ3Qn/hzUXihUHLD12Z"
  "BuzllDf5oYhbzqF0juKlH/PBRMxj2eS+F7TEwuODTr/JF3ORTMLI54MkWsomjw9Fq8MHXPZF33bl"
  "pOcK23F73c71G72JK6/17e7kumO3nYno9W7svsSb/FhGsRcGfMB3tzvXttstVx7zj9VjETWcj4qE"
  "3M84yd5ccXlmTrDmm2LNeCHHdzafivFsPt+n6/lUrOTnLZvPlCkzn2qe/gsAAP//UEsHCFX7E7gl"
  "AwAAvAUAAFBLAwQUAAAAAAAAACE448pmmS0AAAAtAAAAFAAAAE1FVEEtSU5GL01BTklGRVNULk1G"
  "TWFuaWZlc3QtVmVyc2lvbjogMS4wCkNyZWF0ZWQtQnk6IHNvb25nX3ppcAoKUEsBAhQDFAAIAAgA"
  "AAAhOFX7E7glAwAAvAUAAAsAAAAAAAAAAAAAAKSBAAAAAGNsYXNzZXMuZGV4UEsBAhQDFAAAAAAA"
  "AAAhOOPKZpktAAAALQAAABQAAAAAAAAAAAAAAKSBXgMAAE1FVEEtSU5GL01BTklGRVNULk1GUEsF"
  "BgAAAAACAAIAewAAAL0DAAAAAA==";
...
TEST_F(DexFileLoaderTest, ContainerDex) {
  ...
  DexFileLoaderErrorCode error_code;
  std::string error_msg;
  ASSERT_TRUE(OpenDexFilesBase64(
      kRawContainerDex, kLocationString, &dex_bytes, &dex_files, &error_code, &error_msg));
  ASSERT_EQ(dex_files.size(), 2u);

  const DexFile::Header& header0 = dex_files[0]->GetHeader();
  EXPECT_EQ(41u, header0.GetVersion());
  EXPECT_TRUE(header0.HasDexContainer());
  EXPECT_EQ(0u, header0.HeaderOffset());
  EXPECT_LT(header0.file_size_, header0.ContainerSize());

  const DexFile::Header& header1 = dex_files[1]->GetHeader();
  EXPECT_EQ(41u, header1.GetVersion());
  EXPECT_TRUE(header1.HasDexContainer());
  EXPECT_EQ(header0.ContainerSize(), header1.ContainerSize());
  EXPECT_GT(header1.HeaderOffset(), 0u);
  EXPECT_EQ(header1.HeaderOffset(), header0.file_size_);
  EXPECT_EQ(header1.HeaderOffset() + header1.file_size_, header1.ContainerSize());
}
